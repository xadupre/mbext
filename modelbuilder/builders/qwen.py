...
class Qwen35TextModel(Model):
    ...
    def _make_synthetic_position_ids(self):
        ...
        Derives B and S from the ``position_ids`` model input instead of using
        ...
        self.make_shape(shape_name, root_input=pos_ids_input, shape=[in_rank])

        # Slice shape[bs_slice_start:bs_slice_end] → [B, S] (reshape target)
        bs_shape_name = f"{basename}/bs_shape/Slice"
        self.make_slice(
            bs_shape_name,
            inputs=[
                f"{shape_name}/output_0",
                f"/model/constants/INT64/[{bs_slice_start}]",
                f"/model/constants/INT64/[{bs_slice_end}]",
                "/model/constants/INT64/[0]",
            ],
        ...
        # Reshape position_ids → [flat_lead, -1] to get B*S implicitly
        flat_name = f"{basename}/flat/Reshape"
        self.make_reshape(
            flat_name,
            inputs=[pos_ids_input, f"/model/constants/INT64/[{flat_lead}, -1]"],
            dtype=ir.DataType.INT64,
            shape=[flat_lead, "batch_seq"],
        )

        # Shape([flat_lead, B*S]) → [2], Gather scalar index 1 → scalar B*S
        ...
        self.make_gather(
            total_name, inputs=[f"{shape2_name}/output_0", "/model/constants/INT64/1"], dtype=ir.DataType.INT64, shape=[], axis=0
        )
...
class Qwen35MoeTextModel(Qwen35TextModel):
    ...
    def make_moe(self, layer_id, mlp, root_input):
        ...
        # Routed experts (fused layout, with zero expert biases).
        moe_name = self.make_fused_moe(layer_id, mlp, root_input, use_bias=True)
