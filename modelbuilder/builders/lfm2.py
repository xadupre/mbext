...
class LFM2Model(Model):
    ...
    def make_short_conv(self, layer_id, conv_module, root_input):
        ...
        self.make_transpose(
            transpose_1_name,
            f"{in_proj_name}/output_0",
            self.io_dtype,
            shape=["batch_size", 3 * self.hidden_size, "sequence_length"],
            perm=[0, 2, 1],
        )
        ...
        self.make_node(
            "Split", inputs=[f"{transpose_1_name}/output_0", split_tensor_name], outputs=[b_out, c_out, x_out], name=split_name, axis=1
        )
        ...
        self.make_concat(
            conv_input_name,
            [past_conv_name, f"{mul_1_name}/output_0"],
            self.io_dtype,
            shape=["batch_size", self.hidden_size, "total_sequence_length"],
            axis=2,
        )
        ...
        self.make_conv(
            ...
            kernel_shape=[self.conv_L_cache],
            pads=[0, 0],
        ...
        self.make_gather(
            seq_len_gather_name, [f"{shape_name}/output_0", "/model/constants/INT64/1"], dtype=ir.DataType.INT64, shape=[], axis=0
        )

        neg_seq_len_name = f"{basename}/Neg"
        self.make_mul(neg_seq_len_name, [f"{seq_len_gather_name}/output_0", "/model/constants/INT64/-1"], ir.DataType.INT64, shape=[])

        unsqueeze_starts_name = f"{basename}/Unsqueeze"
        self.make_unsqueeze(
            unsqueeze_starts_name, [f"{neg_seq_len_name}/output_0", "/model/constants/INT64/[0]"], ir.DataType.INT64, shape=[1]
        )
    ...
    def make_layer(self, layer_id, layer):
        ...
        self.make_layernorm(layer_id, layer.ffn_norm, skip=True, simple=self.layernorm_attrs["simple"], location="ffn")
