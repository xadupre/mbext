...
class TestTrainedOLMo3Instruct(ExtTestCase):
    def _common_part(self, precision, dtype, provider="cuda"):
        ...
        output_dir, cache_dir = self.get_dirs(
            f"test_trained_allenai_olmo3_7b_instruct_{precision}_{provider}", clean=False
        )
        ...
        * The ONNX logits closely match those of the original PyTorch model.
        ...
        disc.update(
            dict(
                ...
                experiment="forward",
                provider="cpu",
                test="test_trained_allenai_olmo3_7b_instruct_discrepancies_cpu",
                input_type="text",
            )
        )
    ...
    def test_trained_allenai_olmo3_7b_instruct_genai_generate_cuda(self):
        """
        Compare ``transformers.generate`` with ``onnxruntime-genai`` generate
        on the ``allenai/Olmo-3-7B-Instruct`` trained model (fp16, CUDA).
    ...
    def test_trained_allenai_olmo3_7b_instruct_genai_generate_cpu(self):
        """
        Compare ``transformers.generate`` with ``onnxruntime-genai`` generate
        on the ``allenai/Olmo-3-7B-Instruct`` trained model (fp32, CPU).
