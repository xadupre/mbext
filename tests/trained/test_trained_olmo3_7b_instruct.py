...
class TestTrainedOLMo3Instruct(ExtTestCase):
    def _common_part(self, precision, dtype, provider="cuda"):
        ...
        tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME_OLMO3)
    ...
    def _common_trained_discrepancies(self, precision, provider):
        ...
        disc.update(
            dict(
                precision=precision,
                model_id=MODEL_NAME_OLMO3,
                experiment="forward",
                provider="cuda",
                test=f"test_trained_olmo3_7b_instruct_discrepancies_{precision}_{provider}",
                input_type="text",
                kind="prefill",
            )
        )
    ...
    @long_test()
    def test_trained_olmo3_7b_instruct_discrepancies_fp32_cpu(self):
        self._common_trained_discrepancies("fp32", "cpu")

    @long_test()
    @requires_cuda()
    def test_trained_olmo3_7b_instruct_discrepancies_fp16_cuda(self):
    ...
    def _common_trained_generate(self, precision, provider):
        ...
        disc.update(
            dict(
                ...
                experiment="generate",
                provider="cuda",
                test=f"test_trained_olmo3_7b_instruct_genai_generate_{precision}_{provider}",
    ...
    @long_test()
    def test_trained_olmo3_7b_instruct_generate_fp32_cpu(self):
        self._common_trained_generate("fp32", "cpu")

    @long_test()
    def test_trained_olmo3_7b_instruct_generate_int4_cpu(self):
        self._common_trained_generate("int4", "cpu")

    @long_test()
    @requires_cuda()
    def test_trained_olmo3_7b_instruct_generate_fp16_cuda(self):
        self._common_trained_generate("fp16", "cuda")

    @long_test()
    @requires_cuda()
    def test_trained_olmo3_7b_instruct_generate_bf16_cuda(self):
