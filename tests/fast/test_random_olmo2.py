...
class TestOLMo2(ExtTestCase):
    ...
    def test_olmo2_fp32_cpu_random_weights(self):
        """
        Convert a model with the same architecture as OLMo2 but with randomly
        initialised weights to an fp32 ONNX model targeting the CPU execution
        provider.
        ...
        * ONNX logits match PyTorch logits within a small tolerance.
        """
        import torch
        ...
        model_dir = self.get_model_dir("test_olmo2_fp32_cpu_random_weights", clean=False)
        ...
        output_dir, cache_dir = self.get_dirs("test_olmo2_fp32_cpu_random_weights", clean=False)
